  ...
  const patientCheckIn = async () => {
    ...
    try {
      ...
      const qSnap = await getDocs(query(
        ...
        where("hospitalId","==",selHospital),
        where("deptId","==",selDept),
      ));
      const todayCount = qSnap.docs.filter(d=>{
        const ts = d.data().createdAt?.toDate?.();
        return ts && ts >= today;
      }).length;
      const ticket = genTicket(selDept, todayCount);
      ...
      patientName: patientName.trim().substring(0, 100).replace(/[<>{}]/g, ""),
        ...
        checkinType:"self", position:todayCount+1,
        smsConsent: true,
